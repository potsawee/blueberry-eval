...
Usage:
python inference_0shot.py --data_path /nlp/scr/potsawee/workspace/data/LibriSpeech/data_json/test-clean.json --output_dir ./asr_outputs/0shot-blueberry-v1-step238k-temp0.7-top0.8 --temperature 0.7 --top_p 0.8
...
def main() -> None:
    ...
    dataset = load_librispeech_data(args.data_path)
    ...
    for sample in tqdm(dataset):
        ...
        print(f"Running Overall WER: {current_overall_wer:.4f} ({current_overall_wer*100:.2f}%) [%sub={current_sub_pct:.2f}, %ins={current_ins_pct:.2f}, %del={current_del_pct:.2f}] [{samples_processed}/{len(dataset)} samples]")
        sys.stdout.flush()
