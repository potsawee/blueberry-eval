...
def process_one(hypo, truth):
    ...
    # Use modern jiwer API
    output = jiwer.process_words(truth, hypo)
    ref_list = truth.split(" ")
    ref_len = len(ref_list) if len(ref_list) > 0 else 1
    wer = output.wer
    subs = output.substitutions / ref_len
    dele = output.deletions / ref_len
    inse = output.insertions / ref_len
